...
export function DropHero() {
  ...
  return <section className={`brand-opening ${soldOut ? 'is-sold-out' : closed ? 'is-closed' : upcoming ? 'is-upcoming' : ''}`} aria-labelledby="drop-title">
    ...
    <div className="campaign-title page-grid">
      <div className="campaign-eyebrow eyebrow"><span>DROP {drop.number}</span><span>{soldOut ? `${pad(drop.sold)} / ${pad(drop.capacity)}` : upcoming ? 'NEXT DROP' : 'EST. GUATEMALA'}</span></div>
      ...
      <div className="campaign-aside"><p>{soldOut ? `DROP ${drop.number} IS GONE.` : closed ? 'NOW, WE COOK.' : upcoming ? 'SOMETHING GOOD IS COMING.' : drop.tagline}</p><span className="short-rule" /><p>{soldOut ? 'SEE YOU NEXT DROP.' : closed ? 'LA VENTA CERRÓ.' : upcoming ? <>APERTURA DE PEDIDOS<br />{formatOpening(drop)}</> : `${drop.capacity} POR EDICIÓN / SOLO SÁBADO`}</p></div>
    </div>
    <div className="hero-baseline page-grid eyebrow"><span>{soldOut || closed ? 'LIMITED BY DESIGN.' : upcoming ? (drop.ordersOpenAt ? 'HORA DE GUATEMALA' : 'PRÓXIMAMENTE · FECHA POR CONFIRMAR') : 'ONE DROP. ONE VERY GOOD EVENING.'}</span><a href={soldOut || closed || upcoming ? '#next-drop' : '#product'}>{soldOut || closed ? 'EL PRÓXIMO CAPÍTULO' : upcoming ? 'AVISAME DEL DROP' : 'CONOCÉ EL DROP'} <span aria-hidden="true"><ArrowIcon direction="down" /></span></a></div>
  </section>;
}
