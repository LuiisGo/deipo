...
export function PreviewControls() {
  ...
  return <PreviewOnly><details className="preview-controls"><summary>EXPLORAR ESTADOS V0.2 <span aria-hidden="true">+</span></summary><div>
    ...
    <label className="check-row"><input type="checkbox" checked={params.get('opening') === 'demo'} onChange={event => { const query = new URLSearchParams(params); if (event.target.checked) query.set('opening', 'demo'); else query.delete('opening'); router.push(`/?${query}`, { scroll: false }); }} />Apertura de prueba fija: 15 sep. 2026, 00:00 Guatemala</label>
    <label className="check-row"><input type="checkbox" checked={params.get('stock') === 'drift'} onChange={event => { const query = new URLSearchParams(params); if (event.target.checked) query.set('stock', 'drift'); else query.delete('stock'); router.push(`/?${query}`, { scroll: false }); }} />Probar movimiento de stock: +1, +3, +5 a los 12, 24 y 36 segundos. Solo preview.</label>
