...
test('authenticated founder configures, previews and publishes a drop using isolated fixtures',async({page,request})=>{
 ...
 await confirm(page,'Quitar NEXT');await confirm(page,'Asignar CURRENT');await page.goto('/?mode=preview&state=sold_out&stock=drift');await expect(page.locator('body')).toContainText('QA Roast');await expect(page.getByText('STOCK DE PRUEBA')).toHaveCount(0);await expect(page.getByText('SIMULACIÓN INTERNA DE STOCK',{exact:false})).toHaveCount(0);await page.goto('/checkout?mode=preview');await expect(page.getByText('Los pedidos online todavía no están habilitados.')).toBeVisible();
