...
export function ReceiptPrinter() {
  ...
  const home = mode === 'customer-preview' ? '/?mode=customer-preview' : '/';
  const checkout = mode === 'customer-preview' ? '/checkout?mode=customer-preview' : '/checkout';
  const [replay, setReplay] = useState(0);
  const [sound, setSound] = useState(false);
  useEffect(() => () => { void stopPrinterSound(); }, []);
  if (!order) return <main id="main" className="receipt-empty"><DeipoLogo variant="receipt" tone="cream" /><span className="eyebrow">EL RECIBO ES PARTE DEL RITUAL.</span><h1>GOOD FOOD<br />AHEAD<span className="brand-dot">.</span></h1><p>Completá el checkout de prueba para ver tu recibo.<br />Por privacidad, los datos del recibo no se conservan al recargar.</p><Link href={checkout} className="button">PROBAR EL CHECKOUT <span><ArrowIcon /></span></Link><Link href={home} className="text-button">VOLVER AL DROP</Link></main>;
  ...
  return <main id="main" className="receipt-scene">
    ...
    <div className="printer"><div className="printer-edge" aria-hidden="true"><span>deipo. / PRINT STUDIO</span><i /></div><div className="paper-feed" key={replay}>
      <article className="receipt-paper" aria-label="Recibo de demostración">
        <header className="receipt-heading"><DeipoLogo variant="receipt" /><p>FOOD DROPS BY CHEFS<br />FOR WHAT’S NEXT</p></header>
    ...
    <div className="receipt-actions"><button className="text-button" onClick={() => window.print()}>IMPRIMIR RECIBO <ArrowIcon /></button><button className="text-button" onClick={runAgain}>VOLVER A IMPRIMIR <ArrowIcon direction="down" /></button><button className="text-button" aria-pressed={sound} onClick={() => { const next = !sound; setSound(next); if (next) { setReplay(value => value + 1); void playPrinterSound(); } else void stopPrinterSound(); }}>SONIDO: {sound ? 'ACTIVADO' : 'APAGADO'}</button></div>
    ...
    <Link href={home} className="receipt-back text-button"><ArrowIcon direction="left" /> VOLVER AL DROP</Link>
  </main>;
}
