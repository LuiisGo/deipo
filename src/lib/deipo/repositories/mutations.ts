...
export async function mutateDrop(id: string, operation: string, form: FormData) {
  ...
  switch (operation) {
    ...
    case 'inventory': result=await client.from('drops').update({capacity:integer(form,'capacity',1),low_stock_threshold:integer(form,'low_stock_threshold'),max_quantity_per_order:text(form,'max_quantity_per_order')?integer(form,'max_quantity_per_order',1):null}).eq('id',id).select('id').single();break;
