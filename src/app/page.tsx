...
export default async function Home({ searchParams }: { searchParams: Promise<PreviewParams> }) {
  ...
  return <Suspense><SitePresentation mode={getSiteMode(params)}><DropProvider drop={drop}>
    <PresentationNotice />
    <Navigation />
    <main id="main">
      ...
      <DropSection>
        ...
        <CommerceOnly><Reveal className="purchase-panel" event="view_price"><div className="purchase-heading"><span className="eyebrow">DROP {drop.number}</span><span className="eyebrow">{drop.fulfillmentDay} ONLY</span></div><h3>{drop.name}</h3><div className="price-line"><span>{money(drop.price)}</span><span className="caption">POR DROP · TODO INCLUIDO<PreviewOnly><br />PRECIO ESTIMADO</PreviewOnly></span></div><LiveStockIndicator inline /><div className="stock-track" aria-hidden="true"><span style={{ width: `${100 * drop.sold / drop.capacity}%` }} /></div><Countdown /><PurchaseControls /></Reveal></CommerceOnly><ArchiveNote />
      </DropSection>
      <section className="packaging-section page-grid"><div className="packaging-copy"><p className="eyebrow">02 / THE EXPERIENCE</p><h2>{campaign.packagingHeading[0]}<br />{campaign.packagingHeading[1]}</h2><p className="body-copy">{campaign.packagingBody}</p><div className="packaging-note"><span className="seal-mark" aria-hidden="true" /><p className="eyebrow">ONE BOX.<br />ONE SEAL.<br />ONE LIMITED EDITION.</p></div><PreviewOnly><p className="caption">Concepto de empaque. Pendiente de pruebas físicas.</p></PreviewOnly></div><PackagingReveal frames={drop.packagingFrames} /></section>
      ...
      <CommerceOnly><section id="how-it-works" className="logistics-section page-grid"><div className="logistics-heading"><p className="eyebrow">04 / MAKE A LITTLE ROOM</p><h2>YOUR SATURDAY.<br />SORTED.</h2></div><div className="logistics-steps">{[
        ['HASTA EL VIERNES, 11:59 PM', 'Elegí cuántos drops querés. El cierre es el viernes; la mesa te espera el sábado.'],
        ['A TU PUERTA O PARA RECOGER', 'Entrega dentro de nuestra cobertura o pickup. Las zonas se confirman antes de abrir pedidos.'],
        ['ELEGÍ TU HORARIO', 'El sábado, de 6 a 9 PM. Elegí tu ventana de entrega o pickup al hacer el pedido.'],
  ...
  </DropProvider></SitePresentation></Suspense>;
}
