...
test('quantity is limited only by real availability when the per-order cap is null', () => {
  ...
  assert.equal(quantityLimit(currentDrop), 67);
  assert.equal(validateQuantity(10, currentDrop), null);
  assert.notEqual(validateQuantity(68, currentDrop), null);
  for (const value of [0, -1, 1.5, NaN, Infinity]) assert.notEqual(validateQuantity(value, currentDrop), null);
  assert.throws(() => quantityLimit({ ...currentDrop, sold: 81 }), RangeError);
});
...
test('sold out takes precedence and low stock comes from inventory', () => {
  assert.equal(getDropStatus({ ...currentDrop, sold: 80, salesCloseAt: '2020-01-01T00:00:00-06:00' }), 'sold_out');
...
test('mock checkout keeps totals and receipt linked without mutating stock', async () => {
  ...
  if (result.ok) { assert.equal(result.order.total, 350); assert.equal(result.order.contact.name, contact.name); assert.match(result.order.id, /^DEMO-/); assert.equal(result.order.slotLabel, '19:00 — 20:00'); }
...
test('sold-out, expired, malformed contact and simulated payment failure cannot produce receipts', async () => {
  ...
  assert.equal((await completeDemoCheckout({ ...currentDrop, salesCloseAt: '2000-01-01T00:00:00-06:00' }, selection, contact)).ok, false);
