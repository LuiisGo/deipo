...
export function CheckoutShell() {
  ...
  if (!isPurchasable(status)) return <main id="main" className="checkout-unavailable page-grid"><Link href={back}><Wordmark /></Link><span className="eyebrow">DROP {drop.number}</span><h1>{status === 'sold_out' ? 'SOLD OUT.' : status === 'upcoming' ? 'COMING SOON.' : 'ORDERS CLOSED.'}</h1><p>Este drop no está recibiendo pedidos. Te esperamos en el próximo.</p><Link className="button" href={`${back}#next-drop`}>VER EL PRÓXIMO DROP <span>↗</span></Link></main>;
  return <>
    <div className="demo-banner">CHECKOUT DE PRUEBA — NO SE REALIZARÁ NINGÚN COBRO NI RESERVA.</div>
    <header className="checkout-nav page-grid"><Link href={back} aria-label="deipo. inicio"><Wordmark /></Link><Link href={back}>← VOLVER AL DROP</Link><span className="eyebrow">DROP {drop.number}</span></header>
    <main id="main" className="checkout-main page-grid">
      <div className="checkout-form-column"><p className="eyebrow">BUENAS COSAS EN CAMINO.</p><h1>MAKE IT<br />A DATE<span className="brand-dot">.</span></h1>
        ...
        <form onSubmit={next} className="checkout-form" aria-busy={busy}>
          ...
          {step === 3 && <><div className="review-details"><div><span className="eyebrow">A NOMBRE DE</span><p>{contact.name}</p><p>{contact.email}</p><p>{contact.phone}</p><button className="text-button" type="button" onClick={() => goTo(2)}>EDITAR DATOS ↗</button></div><div><span className="eyebrow">{selection.fulfillment === 'delivery' ? 'ENTREGA' : 'PICKUP'}</span><p>{selection.fulfillment === 'delivery' ? contact.address : drop.fulfillment.pickupLabel}</p><p>{drop.fulfillment.slots.find(slot => slot.id === selection.slot)?.label}</p><p>Sábado · fecha por confirmar</p><button className="text-button" type="button" onClick={() => goTo(1)}>EDITAR ENTREGA ↗</button></div></div><div className="mock-payment"><span className="eyebrow">VISTA PREVIA DEL PAGO</span><p>Sin tarjeta. Sin cobro.</p><span className="caption">El siguiente paso genera un recibo de demostración. No confirma una compra real.</span></div><label className="check-row consent-row"><input type="checkbox" checked={accepted} onChange={event => setAccepted(event.target.checked)} required /><span>Entiendo que es una prueba y que no se realizará un pedido.</span></label><p className="caption">Podés revisar los borradores de <Link href="/legal/orders">política de pedidos</Link> y <Link href="/legal/privacy">privacidad</Link>.</p></>}
          {error && <p className="form-error" role="alert">{error}</p>}
          <div className="checkout-actions">{step > 0 && <button disabled={busy} type="button" className="text-button" onClick={() => goTo(step - 1)}>← ATRÁS</button>}<button type="submit" className="button" disabled={busy || (step === 1 && !drop.fulfillment.slots.some(slot => slot.available))}>{busy ? 'PREPARANDO TU RECIBO…' : step === 3 ? 'COMPLETAR PRUEBA' : 'CONTINUAR'}<span aria-hidden="true">↗</span></button></div>
        </form>
      </div>
      <aside className="order-summary"><Media src={drop.heroImage} alt="Sunday Roast, fotografía ilustrativa del drop." className="summary-image" sizes="(max-width: 900px) 100vw, 35vw" /><div className="summary-content"><span className="eyebrow">DROP {drop.number} / TU PEDIDO</span><h2>{drop.name}</h2><div className="summary-line"><span>{selection.quantity} × drop completo</span><span>{money(drop.price * selection.quantity)}</span></div>{drop.extras.filter(extra => selection.extras.includes(extra.id)).map(extra => <div className="summary-line" key={extra.id}><span>{extra.name}</span><span>{money(extra.price)}</span></div>)}<div className="summary-line"><span>{selection.fulfillment === 'delivery' ? 'Entrega' : 'Recoger'}</span><span>{price.deliveryFee === null ? 'Por confirmar' : price.deliveryFee === 0 ? 'Incluido' : money(price.deliveryFee)}</span></div><div className="summary-total"><span>TOTAL ESTIMADO</span><strong>{price.total === null ? 'Por confirmar' : money(price.total)}</strong></div><p className="caption">Todos los valores son de prueba. No se realizará ningún cobro.</p><p className="eyebrow summary-signoff">GOOD FOOD.<br />GOOD PLANS.</p></div></aside>
    </main>
  </>;
}
